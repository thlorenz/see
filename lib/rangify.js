...
function subLocate(ranges, line) {
  ...
  return areEqual(ranges[stopperBwd], range) && areEqual(ranges[stopperFwd], range)
    // stoppers are head and tail of a function, if/else, etc.
    ? [ stopperBwd, stopperFwd ]
    // one of the stoppers is not part of the enlosing function, if/else, etc.
    : [ stopperBwd + 1, stopperFwd - 1 ];
}
...
var result = rangify(sectionized);
console.log(result);
